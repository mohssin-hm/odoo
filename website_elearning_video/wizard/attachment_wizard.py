...
class VideoAttachmentWizard(models.TransientModel):
    ...
    def add_video_attachment(self):
        ...
        allowed_types = self.env['website.slide.video'].search([('is_active','=',True)],limit=1)
        allowed_format = []
        for format in allowed_types.allowed_types:
            allowed_format.append(format.name) 

        if (res.mimetype in allowed_format and res.file_size <= file_size):
            attachmentObj = self.env['slide.slide'].browse(modelId)
            attachmentObj.slide_attachment = res.id
        
        else:
            res.unlink()
            raise UserError(_("Only allowed formats and size less than 25Mb can be uploaded."))
