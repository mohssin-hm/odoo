...
class SaleOrderUpdate(models.Model):
	...
	def _computetotal(self):
		for record in self:
			invoice_id = self.env['account.move'].search(['&',('invoice_origin','=', record.name),'|',('state','=','posted'),('invoice_payment_state','=','paid')])
			total = 0
	...
	def _computedue(self):
		for record in self:
			item_id = self.env['account.move'].search(['&',('invoice_origin','=', record.name),'|',('state','=','posted'),('invoice_payment_state','=','paid')])
			aggregate = 0
			for comp in item_id:
				aggregate +=comp.amount_residual

			record.amount_due = aggregate
